...
def parse_args():
    ...
    parser.add_argument('-c', '--config', required=True, help='Path to Darknet .cfg file')
    parser.add_argument('-w', '--weights', required=True, help='Path to Darknet .weights file')
    parser.add_argument('-o', '--output_dir',
        default=Path('/workspace/output'),
        help='Path to output directory for .onnx file')
...
def main():
    ...
    output_tensor_shapes = [
        [c, h // 8, w // 8], [c, h // 16, w // 16], [c, h // 32, w // 32], [c, h // 64, w // 64], [c, h // 128, w // 128]]
    if len(output_tensor_names) == 2: # tiny
        output_tensor_shapes = output_tensor_shapes[1:]
    output_tensor_shapes = output_tensor_shapes[:len(output_tensor_names)]
    if not is_pan_arch(args.config):
