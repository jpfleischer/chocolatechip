...
def heatmap_generator(df_type: str,
    ...
    if df_type == 'track':
        ...
        if p2v:
            ...
            # Function to process the looked-at variable
            ...
            for index, row in omega.iterrows():
                if 'ped' in row['cluster1']:
                    process_variable(row['cluster2'])
                elif 'ped' in row['cluster2']:
                    process_variable(row['cluster1'])
