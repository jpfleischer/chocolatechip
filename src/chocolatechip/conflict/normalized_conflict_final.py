...
# Combine before and after average conflict rates per 1,000 vehicles
combined_rates_vehicle = pd.concat([
    merged_before_vehicle[['day_of_week', 'average_conflicts_per_1000_vehicle']],
    merged_after_vehicle[['day_of_week', 'average_conflicts_per_1000_vehicle']]
], ignore_index=True)['average_conflicts_per_1000_vehicle'].values

# Combine before and after average conflict rates per 1,000 pedestrians
combined_rates_pedestrian = pd.concat([
    merged_before_pedestrian[['day_of_week', 'average_conflicts_per_1000_pedestrian']],
    merged_after_pedestrian[['day_of_week', 'average_conflicts_per_1000_pedestrian']]
], ignore_index=True)['average_conflicts_per_1000_pedestrian'].values

# Create time vector (1 per weekday for both before and after)
time_vector = np.arange(len(combined_rates_vehicle))

# Calculate correlation
vehicle_corr, vehicle_pval = pearsonr(time_vector, combined_rates_vehicle)
ped_corr, ped_pval = pearsonr(time_vector, combined_rates_pedestrian)

print(f"Correlation for P2V Conflicts per 1000 Vehicles Over Time: r = {vehicle_corr:.3f}, p = {vehicle_pval:.3f}")
print(f"Correlation for P2V Conflicts per 1000 Pedestrians Over Time: r = {ped_corr:.3f}, p = {ped_pval:.3f}")

# Perform t-tests to compare before and after conflict rates
vehicle_rate_before = merged_before_vehicle['average_conflicts_per_1000_vehicle'].values
vehicle_rate_after = merged_after_vehicle['average_conflicts_per_1000_vehicle'].values

ped_rate_before = merged_before_pedestrian['average_conflicts_per_1000_pedestrian'].values
ped_rate_after = merged_after_pedestrian['average_conflicts_per_1000_pedestrian'].values

# Two-sample t-tests (unequal variances)
t_stat_vehicle, p_val_vehicle = ttest_ind(vehicle_rate_before, vehicle_rate_after, equal_var=False)
t_stat_pedestrian, p_val_pedestrian = ttest_ind(ped_rate_before, ped_rate_after, equal_var=False)

print(f"\nT-Test: Average P2V Conflicts per 1,000 Vehicles")
print(f"  t-statistic: {t_stat_vehicle:.3f}, p-value: {p_val_vehicle:.3f}")
print("  Interpretation:", "Significant" if p_val_vehicle < 0.05 else "Not significant")

print(f"\nT-Test: Average P2V Conflicts per 1,000 Pedestrians")
print(f"  t-statistic: {t_stat_pedestrian:.3f}, p-value: {p_val_pedestrian:.3f}")
print("  Interpretation:", "Significant" if p_val_pedestrian < 0.05 else "Not significant")
